 ...
 */
 ...
 $nom = $_POST['Nom'];
$email = $_POST['email'];
// Vérifie que les autres champs sont correctement récupérés
...
require 'vendor/autoload.php'; // Assure-toi que PHPMailer est installé avec Composer, ou ajuste le chemin si tu as téléchargé les fichiers manuellement
...
try {
    ...
    $mail->isSMTP();  // Indiquer que nous allons utiliser SMTP
    $mail->Host = 'smtp.gmail.com';  // Serveur SMTP de Gmail
    $mail->SMTPAuth = true;  // Authentification SMTP
    $mail->Username = 'comnkongo@gmail.com';  // Ton adresse email Gmail
    $mail->Password = '';  // Ton mot de passe Gmail (ou mot de passe d'application si tu utilises l'authentification 2 facteurs)
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Sécuriser la connexion
    $mail->Port = 587;  // Port pour TLS (587)

    // Expéditeur et destinataire
    $mail->setFrom('comnkongo@gmail.com', 'Ton nom');  // Ton adresse email Gmail comme expéditeur
    $mail->addAddress('destinataire@example.com', 'Nom du destinataire');  // L'email du destinataire

    // Contenu du message
    $mail->isHTML(true);  // Spécifie que le message est en HTML
    $mail->Subject = 'Sujet de l\'email';  // Le sujet de l'email
    $mail->Body = 'Contenu de l\'email';  // Le corps de l'email

    // Débogage (active cette option pour voir les messages d'erreur détaillés)
    $mail->SMTPDebug = 2;  // Niveau de débogage (2 = informations complètes)

    // Envoi du message
    if ($mail->send()) {
        echo 'Message envoyé avec succès';
    } else {
        echo 'Erreur: ' . $mail->ErrorInfo;  // Afficher l'erreur en cas de problème
    }
} catch (Exception $e) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $nom = isset($_POST['nom']) ? $_POST['nom'] : 'Non précisé';
    $email = $_POST['email']; 
    ...
    $appareils_endommages = isset($_POST['appareils_endommages']) ? implode(', ', $_POST['appareils_endommages']) : 'Aucun';
    $quantite_appareils_endommages = $_POST['quantite_appareils_endommages'];
    $commentaires = $_POST['commentaires'];

    // Traiter les fichiers téléchargés
    $fichiers_images = $_FILES['images_appareil'];
    $fichiers_documents = $_FILES['documents'];
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'comnkongo@gmail.com'; 
        $mail->Password = '@Lionel26';
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;

        // Expéditeur et destinataire
        $mail->setFrom('comnkongo@gmail.com', 'Nom de l\'expéditeur');
        $mail->addAddress('comnkongo@gmail.com', 'Commnunication de Nkongô');
        
        // Sujet et contenu de l'email
        $mail->Subject = 'Sondage SEEG Port-Gentil - Réponse';
        $mail->isHTML(true);
        $mail->Body = "<h3>Détails du Sondage :</h3>
                       <strong>Nom :</strong> $nom<br>
                       <strong>Email :</strong> $email<br>
                       <strong>Téléphone :</strong> $telephone<br>
                       <strong>Temps de client :</strong> $temps_utilisation<br>
                       <strong>Arrondissement :</strong> $arrondissement<br>
                       <strong>Quartier :</strong> $quartier<br>
                       <strong>Date de problème :</strong> $date<br>
                       <strong>Qualité de l'électricité :</strong> $qualite_electricite<br>
                       <strong>Coupures fréquentes :</strong> $coupures<br>
                       <strong>Qualité de l'eau :</strong> $qualite_eau<br>
                       <strong>Appareils endommagés :</strong> $appareils_endommages<br>
                       <strong>Quantité d'appareils endommagés :</strong> $quantite_appareils_endommages<br>
                       <strong>Commentaires :</strong> $commentaires";

        // Ajouter les images et documents en pièces jointes
        foreach ($fichiers_images['tmp_name'] as $key => $tmp_name) {
            if ($fichiers_images['error'][$key] === UPLOAD_ERR_OK) {
                $mail->addAttachment($tmp_name, $fichiers_images['name'][$key]);
            }
        }

        foreach ($fichiers_documents['tmp_name'] as $key => $tmp_name) {
            if ($fichiers_documents['error'][$key] === UPLOAD_ERR_OK) {
                $mail->addAttachment($tmp_name, $fichiers_documents['name'][$key]);
            }
        }
